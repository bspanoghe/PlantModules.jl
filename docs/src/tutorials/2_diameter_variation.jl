...
- Defining a large plant structure with the use of [PlantGraphs.jl](https://github.com/VirtualPlantLab/PlantGraphs.jl)'s L-system modeling framework.
...
md"""
In this tutorial, we recreate [the dynamic sap flow model by Perämäki et al. (2001)](https://doi.org/10.1093/treephys/21.12-13.889) in PlantModules.jl. This model is an interesting case study for a few reasons:
- It is also based on hydraulics-based plant growth, so we can use the pre-built core functionality from the package.
- The structure is composed of about 100 segments from two different types, which lends itself well to a modular modeling approach.
...
- The paper includes input- and output data, allow for model validation.
"""
...
branching_rule = Rule( 
	# top stem segment grows another thinner stem segment and a branch every `branching_frequency` steps, if stem is old enough to form crown
    StemTip,
...
vertical_growth_rule = Rule( 
	# top stem segment grows only another stem segment if not branching
    StemTip,
    lhs = st -> !(data(st).age >= crown_start_age && data(st).age % 
		branching_frequency == 0),
    rhs = st -> Stem(data(st).D, data(st).height) + 
...
branchgrowth_rule = Rule( 
	# top branch segments grown another branch segment every `branchgrowth_frequency` steps
    BranchTip,
...
# ╔═╡ ad30a47f-1cc1-4d76-b7e8-0a98de3802af
md"Transpiration rate from data"
...
function fixed_transpiration_connection(; name)
    @constants begin
        t_unit = 1, [description = "Dummy constant for correcting units", 
					 unit = u"hr"]
        uc = 1e-3 * 3600 * (1e-2)^2, [description = 
			"Unit conversion from (mg / s / m^2) to (g / hr / cm^2)",
			 unit = u"g / hr / cm^2"]
    end
    @variables begin
        F_s(t), [description = 
			"Specific water flux from compartment 2 to compartment 1", 
				 unit = u"g / hr / cm^2"]
